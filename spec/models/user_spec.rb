...
RSpec.describe User, type: :model do
  ...
  describe 'Validations' do
    ...
    let(:user) {User.new(name: "Tin Tin", email: 'tintin@t.Com', password:'1234', password_confirmation: '1234')}
    ...
    it "must have unique emails" do
      ...
      @user2 = User.create(name: "Tin Tin", email: 'tintin@t.Com', password:'1234', password_confirmation: '1234')
      expect(User.where(email: user.email).count).not_to be(2)
    end

    it "emails can not be case sensitive" do
      user.save
      @user3 = User.create(name: "Tin Tin", email: 'tintin@T.Com', password:'1234', password_confirmation: '1234')
  ...
  describe '.authenticate_with_credentials' do
    let(:user) {User.create(name: "Tin Tin", email: 'tintin@t.Com', password:'1234', password_confirmation: '1234')}
    ...
    it "should authenticate the user if there's a wrong case in the email" do
      @user = User.authenticate_with_credentials('TinTin@T.Com', user.password)
      expect(@user).not_to be_nil
    end
